...
async function exportPdf() {
  ...
  try {
    ...
    const images = Array.from(reportPages.querySelectorAll('img'));
    await Promise.all(
      images.map((img) => (img.complete ? Promise.resolve() : img.decode().catch(() => null)))
    );
    await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
    const pageCount = reportPages.querySelectorAll('.a4-page').length || 1;
    const rect = reportPages.getBoundingClientRect();
    const canvasWidth = Math.ceil(rect.width);
    const canvasHeight = Math.ceil(canvasWidth * (297 / 210) * pageCount);
    const options = {
      margin: [0, 0, 0, 0],
